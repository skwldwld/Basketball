...
function init() {
  ...
  const floorVisualMaterial = new THREE.MeshStandardMaterial({
    color: 0xdeb887,
  });
  ...
  floorPhysicsMaterial = new CANNON.Material("floor");
  ...
  objloader.load(
    hoopObjPath,
    (object) => {
      const texture = textureLoader.load(hoopTexturePath);
    ...
    undefined,
    (error) => {
      console.error("골대 모델 로드 실패:", error);
    }
  );
  ...
  objloader.load(
    stadiumObjPath,
    (object) => {
      const texture = textureLoader.load(stadiumTexturePath);
    ...
    undefined,
    (error) => {
      console.error("농구 경기장 모델 로드 실패:", error);
    }
  );
  ...
  gltfLoader.load(
    ...
    undefined,
    (error) => {
      console.error("관중석 모델 로드 실패:", error);
    }
  );
  ...
  gltfLoader.load(
    ...
    undefined,
    (error) => {
      console.error("하늘 모델 로드 실패:", error);
    }
  );
  ...
  for (let i = 0; i < segments; i++) {
    ...
    const shape = new CANNON.Box(
      new CANNON.Vec3(pieceSize / 2, ringHeight, pieceSize / 2)
    );
  ...
  const poleShape = new CANNON.Box(
    new CANNON.Vec3(poleWidth / 2, poleHeight / 2, poleDepth / 2)
  );
  ...
  document.addEventListener("keyup", (event) => {
    if (event.code === "Space" && isCharging && !isThrown) {
      if (!ballBody) {
        console.warn("공 물리 바디가 아직 준비되지 않았습니다.");
        isCharging = false;
        return;
      }
      ...
      ballBody.addEventListener("collide", (event) => {
        ...
        if (
          destroyableBalls.includes(ballName) &&
          impactVelocity > breakThreshold
        ) {
          destroyBall();
        }
      });
...
function loadPlayerBallModel(playerIndex) {
  if (!selectedBalls[playerIndex]) {
    console.warn("해당 플레이어의 공 정보가 없습니다.");
    return;
  }
  ...
  const ballModelPath = ballModelPaths[ballData.model];

  if (!ballModelPath) {
    console.warn(`공 모델 경로를 찾을 수 없습니다: ${ballData.model}`);
    return;
  }
...
function destroyBall() {
  ...
  function animateParticles(time) {
    ...
    particleGroup.children.forEach((particle) => {
      ...
      particle.material.opacity = Math.max(0, 1 - elapsed / duration);
    });
...
function proceedToNextTurn() {
  ...
  if (remainingThrows.every((t) => t <= 0)) {
    localStorage.setItem("selectedBalls", JSON.stringify(selectedBalls));
    window.location.href = `${import.meta.env.BASE_URL}result.html`;
    return;
  }
...
function animate() {
  ...
  if (isThrown && ballBody) {
    ...
    if (withinHoopRadius && passedThroughHoop) {
